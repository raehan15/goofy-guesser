...
function GameContent() {
  ...
  const [guessStates, setGuessStates] = useState<LetterState[][]>(
    Array(6).fill(Array(5).fill("initial")),
  );
  ...
  const initializeFromProgress = useCallback(
    (progress: {
      guesses: string[];
  ...
  const handleGameEnd = useCallback(async () => {
    ...
    if (result.errors.length > 0) {
      ...
      alert(
        "Your result could not be fully submitted. Please reopen the game and try again.",
      );
    }
  ...
  if (view === "groups" && user) {
    ...
    const playedTodayStatus =
      isCheckingPlayStatus || groupsLoading || authLoading
        ? null
        : hasPlayedToday || resultSubmitted
          ? true
          : false;
    ...
    return (
      <div
        style={{
          minHeight: "100dvh",
          background:
            "linear-gradient(165deg, #0a0f0f 0%, #0d1717 30%, #0f1a1a 60%, #0a1212 100%)",
        }}
      >
        <Navbar />
        <GroupsPanel
